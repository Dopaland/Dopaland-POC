...
reclassification (`D0PA1_Client_SignOff_001.md` §4, 2026-09-19) — see the
...
`docs/preregistration/D0PA1_Client_SignOff_001.md` §4 (signed 2026-09-19)**
...
| 7 | Gate 3 rule | RETURNED | DEFINED-NOT-IMPLEMENTED | Yes | Formula accepted from the client's own §5.3 text; `δ_Gate3 = 0.05 nats`, derived from context-baseline information gain (§4.7), **ACCEPTED by the client** (`D0PA1_Client_SignOff_001.md` §2, 2026-09-19) and now a frozen field in `simulation/config.py`'s `PreRegisteredConfig` — no code computes the rule itself, correctly (G1; no real M_core/M0b exists). |
...
| 15 | Positive control (blink) | BUILT, NOT YET RUN ON REAL DATA | IMPLEMENTED-BUT-NEVER-RUN-ON-REAL-DATA | Yes | `controls/blink_positive.py`; F1 1.0/0.615 and the AST criterion-check both re-verified live this session. Criterion (F1≥0.80, count within ±20% on ≥8/10 clips, ±150ms matching tolerance) **ACCEPTED by the client** (`D0PA1_Client_SignOff_001.md` §3, 2026-09-19), values unchanged from what was already stored. **Not run in this engagement (the human's decision, 2026-09-25).** Every technical prerequisite was met and demonstrated: the committed runner `controls/run_blink_detector.py` (`739fa48`); the per-frame aperture export (`beb8843`); the aperture path on real footage (`P01_clip01`: 98.15% non-`None` apertures, SD 0.0371 across 984 distinct values, which shows the stream carries real values from a real face and **says nothing about whether blink detection works**); ten clips from ten subjects, recorded and kept outside the repository under the retention in `RETENTION.txt`; a frozen counting rule (v2, reopen edge; its "frozen on" date field was left blank); ten blank manual-count templates. The detector ran once on `P01_clip01`, as a pipeline demonstration from an uncommitted tree; that output was never viewed and is not citable. **What is missing is the blind manual count**, roughly an hour of frame-by-frame human work that was not carried out. That is not a technical blocker, a tooling gap or a pipeline limitation. **No automated count was substituted.** The control compares a detector against an independent human count made blind; an automated count would compare one automated estimate against another with no independent ground truth, and would make the pipeline score itself. That is the failure G1 exists to prevent, and the same reason Gate 2, the soak and the δ thresholds are applied by a human against a rule frozen beforehand. **No PASS, FAIL or partial result exists**: the criterion was never applied, because there is no count to apply it to. It stays frozen and signed, and applies unchanged if the control is ever run. The control remains **runnable later**, from any machine: the repository, runner and criterion are on the remote, and the clips are kept under retention. This is "not run", not "cannot be run". |
...
| 19 | Modality ablation | RETURNED | DEFINED-NOT-IMPLEMENTED | Yes | **Reclassified from DECISION REQUIRED, confirmed** (`D0PA1_Client_SignOff_001.md` §4, 2026-09-19) — the contingency on Decision A is discharged, the client retained Δ_audio. `δ_audio = 0.05 nats` accepted (§2) on the same reasoning as the other components. Note the dependency, unchanged by the reclassification: the ablation itself cannot be run until a clock-synchronisation measurement exists, and that measurement is now a documented omission (CC-001 §6(a), row 23) — the reclassification reflects the decision and definition being complete, not that the ablation has been run. |
